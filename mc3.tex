...
%
Mixing refers to how independent samples are in a chain,
and we assessed it by measuring a chain's effective size,
which is the size of the chain adjusted for autocorrelation.
%
Effective size was calculated using the coda library (version 0.16-1)
in R (version 3.0.1).
%
Convergence refers to how well the chain approximates the true distribution,
and we assessed it by calculating the variance in the number of shifts
among runs.
%
The assumption is that runs that have not converged
will have a more variable number of estimated shifts
compared to runs that have converged.
%
Finally, we compared the length of time runs
...
with four and eight chains was 548 (494--603) and 555 (494--611),
whereas with one chain it was 402 (331--459).
%
For the skink tree,
the mean effective size with four chains was 601 (558--644),
whereas with one chain it was 290 (205--380).
%
