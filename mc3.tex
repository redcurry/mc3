...
If an MCMC simulation samples from the target distribution
...
For each treatment,
we measured the variance of the median log-likelihoods
(20\% burn-in) sampled by each replicate MCMC simulation.
%
We used the median log-likelihood as a summary statistic
for the distribution sampled by an MCMC simulation.
%
To statistically compare these variances between treatments,
we calculated the 5\% lower and upper bounds of 10,000 variances,
each calculated from a random sample of the replicate variances.
